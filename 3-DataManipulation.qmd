...
```{r}
#What is the percentage of death as a termination?
```
...
#I'm curious about how many of these women were single because many of them were young, it's impossible to tell from the data set however I think it's possible to get an idea. 
data(BostonWomenVoters)
women<-BostonWomenVoters %>%
  filter(Name != "") %>%
  group_by(Name)%>%
  summarize(count = n()) %>%
  summarize(sum(count))
...
husbands<-BostonWomenVoters%>%
group_by(Husband.Name) %>%
  filter(Husband.Name != "")%>%
  summarize(count = n()) %>%
  summarize(sum(count))

print(paste("The total number of women is", women, "and the total number of women who did not fill out the husband name block is", women-husbands))
#obviously this means very little though it does show an interesting fact about the data, either married women did not care to write their husbands name down or single women were more likely to register to vote
```
