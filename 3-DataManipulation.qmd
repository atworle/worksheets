...
>I used '<-' then piped select from the gayguides dataset. If I don't assign it to a variable I would have to run the line of code over and over to get the data it specifies.  
...
```{r}
nocs<- gayguides %>% select(-city, -state)
```
...
```{r}
gayguides %>%
  filter(city == "Greenville" & state == "SC" & Year %in% 1975:1980)
```

(@) Can you find all locations in 1975 except for New York and San Francisco? 
```{r}
x<-gayguides %>% 
  filter(Year == 1975 & city != "NY" & city != "SF")
```
...
```{r}
data(gayguides)
gayguides %>% mutate(print(paste(city,",", state)))
```
...
data(gayguides)
gayguides %>%
  filter(state == "SC" & Year %in% 1970:1975) %>%
  group_by(city) %>%
  summarize(count = n()) %>%
...
joinedgg<-left_join(gayguides, regions, by=c("state" = "State.Code"))
```

(@) Explain what you did above. What variable did you join by and why? What results?
I made a variable for the joined data set so it would be easier to access. I choose left_join because I felt the data in regions would fit better if it was to the right of the gayguides data set. Since the state abbrevations were what the data sets had in common I choose that as the variable to join. The results have the region and division from the regions data in the right most columns of gayguides.
...
```{r}
growth.by.state<-joinedgg %>% 
  filter(Year %in% 1970:1980) %>%
  group_by(state) %>% 
  summarize(count = n()) 

growth.by.region<- joinedgg %>%
  filter(Year %in% 1970:1980) %>%
  group_by(Region) %>%
  summarize(count = n())
...
>The code turned the columns, excluding city and type_of_worker, to one column named year and then the values in those columns into a column named count. 
...
```{r}
#What percent of termination was recorded as death?
```
...
data(judges)
judges %>%
  group_by(Termination..1.) %>%
  filter(Termination..1. != "")%>%
  #here i'm just trying to filter out the blank cells
  summarize(count = n()) %>%
  mutate(percentage = (count/sum(count) * 100)) %>%
  #here using a simple percentage equation with the calculation from previous summarize
  arrange(desc(count))
  #arranging here so I can instantly see the highest percentage
...
```{r}
#One question about the suffrage movement I always have is: were these highly centralized groups of women or was it more web-like and loosely connected?
#Are these women geographically connected or isolated?
```
...
BostonWomenVoters %>%
  group_by(Street.of.Residence.on.April.1, Occupation) %>%
  #the reason I'm pairing with occupation is because I believe occupation offers more historical information than anything else in the dataset
  summarize(count = n()) %>%
  arrange(desc(count))
  #once again desc here because there's so many entries and I want to see the numbers that matter
  #my data reveals extremely interesting relationships between occupation and where these women lived. For example, 20 Stenographers lived on the same street, that's fascinating. Also 131 women who registered to vote lived on Saratoga Street. Is this data meaningful or can it be attributed to women grouping up knocking on doors and walking down to get registered? The biggest overlap was between housewifes and streets of residences, there were 10 streets that had hundreds of women. This perhaps points towards voter registration as social activity.

business.place<-BostonWomenVoters %>%
  group_by(Place.of.Business)%>%
  summarize(count = n()) %>%
  arrange(desc(count))

  #I was curious to see how place of business played into this and it was extremely decentralized but that is probably just a reflection of the socioeconomics of this time period. However, there were repsonses that had "same" for this block which I would take to mean the responses with "same" was recorded at the same time as the one above it. This implies that at least two women went together to register. Was registering to vote a social activity?
...
```{r}
data(gayguides)
grab.by.state<- function(Year,state) {
  gayguides %>%
